...
local descr = "An open source machine learning framework that accelerates the path from research prototyping to production deployment."
...
add_versions("v2.6.0", "1eba9b3aa3c43f86f4a2c807ac8e12c4a7767340")

-- patches
-- patches/1.12.1/clang.patch: fix compile on macOS (https://github.com/pytorch/pytorch/pull/80916)
...
-- xmake package configs
-- only ninja is supported on windows (https://github.com/pytorch/pytorch/issues/24186)
add_configs("cmshared", { description = "Build torch's shared libraries.", default = true, type = "boolean" })
...
on_install("windows|x64", "macosx", "linux", function(package)
    ...
    -- windows compiler version check
    if package:is_plat("windows") then
        local vs = toolchain.load("msvc"):config("vs")
        if tonumber(vs) < 2019 then
            raise("Your compiler is too old to use this library. Use msvc >= 2019")
        end
    end

    -- built library file suffixes
    local staticlib_suffix = ".a"
    if package:is_plat("windows") then
        staticlib_suffix = ".lib"
    end

    local sharedlib_suffix = ".so"
    if package:is_plat("macosx") then
        sharedlib_suffix = ".dylib"
    end

    -- links and link flags
    if not package:is_plat("windows") and package:config("cmshared") then
        package:add("ldflags", "-Wl,-rpath," .. package:installdir("lib"))

        local libdir = package:installdir("lib", "lib")
        local fl_rpath = package:is_plat("linux") and "-Wl,-rpath," or ""
        local fl_noas = package:is_plat("linux") and "-Wl,--no-as-needed," or ""
        local fl_as = package:is_plat("linux") and "-Wl,--as-needed," or ""
        for _, lib in ipairs({ "torch", "kineto", "c10", "c10_cuda" }) do
            package:add("ldflags", fl_rpath .. libdir .. lib .. sharedlib_suffix)
        end
        for _, lib in ipairs({ "torch", "torch_cpu", "torch_cuda" }) do
            package:add("ldflags", fl_noas .. libdir .. lib .. sharedlib_suffix)
        end
        for _, lib in ipairs({ "c10", "c10_cuda" }) do
            package:add("ldflags", fl_as .. libdir .. lib .. sharedlib_suffix)
        end
        for _, lib in ipairs({ "torch", "c10", "c10_cuda" }) do
            package:add("links", lib)
        end
    end

    -- build torchlibs in static mode
    if not package:config("cmshared") then
        local libs = {
            "torch",
            "torch_cpu",
            "c10",
        ...
        if package:config("cuda") then
            table.insert(libs, "torch_cuda")
            table.insert(libs, "c10_cuda")
        end
    ...
    -- patch some third-party cmake files
    ...
    -- prepare python
    -- requires {typing_extensions, pyyaml} are installed in python build env
    local python_exe = package:is_plat("windows") and "python" or "python3"
    local pythonpath, _ = os.iorun(python_exe .. " -c \"import sys; print(sys.executable)\"")
    ...
    table.insert(configs, "-DBUILD_SHARED_LIBS=" .. (package:config("cmshared") and "ON" or "OFF"))
    ...
    if package:config("python") then
        -- requires {numpy} is installed in python build env
    ...
    if package:is_plat("windows") then
        local mtruntime = package:config("vs_runtime"):startswith("MT")
        table.insert(configs, "-DCAFFE2_USE_MSVC_STATIC_RUNTIME=" .. (mtruntime and "ON" or "OFF"))
        table.insert(configs, "-DCPUINFO_RUNTIME_TYPE=" .. (mtruntime and "static" or "shared"))
    ...
    for _, libname in ipairs(cp_libs) do
        os.trycp(path.join(package:buildir(), "lib", libname .. staticlib_suffix), package:installdir("lib"))
    end

    -- patch the resulting TorchConfig.cmake so it will find statically-linked libs correctly
    io.replace(
